...
tiemposParaOrdenes ordenes = map  cuantoTeLleva ordenes

cuantoTeLleva orden
   | tienePocoTrabajo ordenesPasteleria = 2 * cantidad orden
   | otherwise = 3 * cantidad orden

clienteContento (c, crit) ordenes = cuantoTeLlevaHasta (ordenDelCliente c ordenes) ordenes  < cuantoPuedoTardar (c, crit)

ordenDelCliente cli ordenes = (head.filter ((==cli).nombre)) ordenes

cuantoTeLlevaHasta orden ordenes = (sum.take  (pos orden ordenes).tiemposParaOrdenes) ordenes
